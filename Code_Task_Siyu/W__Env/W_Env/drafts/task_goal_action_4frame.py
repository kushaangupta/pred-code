...
class task_MC_4frame(W_Gym_grid2D):
    p_reward = None
    def __init__(self, p_reward = 1, n_maxTrialsPerBlock = 30, **kwarg):
        super().__init__(3,3,4, is_ITI = False, n_maxTrialsPerBlock = n_maxTrialsPerBlock, **kwarg)
        self.p_reward = p_reward
        # set action space
        self.action_space = spaces.Discrete(5) # stay, left, up, right, down
        # set rendering dimension names
        self.setup_obs_Name2DimNumber({'fixation':0, 'square':1, 'imageA':2, 'imageB':3})
        # set stages
        stage_names = ["flash1", \
                       "flash2", "choice1", \
                        "choice2"]
